...
from renderers.emoji_icons import emoji_icon
...
async def create_donation_image(
    ...
    *,
    ...
    for idx, player in enumerate(leaderboard[:15], start=1):
        ...
        medal = (
            emoji_icon("🥇", "gold_medal", class_name="rank-icon") if idx == 1 else
            emoji_icon("🥈", "silver_medal", class_name="rank-icon") if idx == 2 else
            emoji_icon("🥉", "bronze_medal", class_name="rank-icon") if idx == 3 else
            f"#{idx}"
        )
        ...
        <div class="donation-row{row_class}">
          ...
          <div class="donation-stats">
            <div><span>{emoji_icon("📦", "donated", class_name="stat-icon")}</span> {_fmt_num(donated)}</div>
            <div><span>{emoji_icon("📥", "received", class_name="stat-icon")}</span> {_fmt_num(received)}</div>
            <div><span>{emoji_icon("📊", "ratio", class_name="stat-icon")}</span> {ratio}</div>
          </div>
        </div>
        """
...
<style>
  * {{ box-sizing: border-box; }}
  ...
  .title {{ position: relative; z-index: 1; font-size: 42px; font-weight: 900; line-height: 1; text-shadow: 0 4px 0 rgba(0,0,0,.34), 0 0 12px rgba(255,255,255,.10); display: flex; align-items: center; gap: 10px; }}
  ...
  .leaderboard h2 {{ margin: 0 0 12px; font-size: 27px; font-weight: 900; text-shadow: 0 3px 0 rgba(0,0,0,.48); display: flex; align-items: center; gap: 8px; }}
  ...
  .donation-rank {{ font-size: 24px; font-weight: 900; text-align: center; text-shadow: 0 3px 0 rgba(0,0,0,.34); display: flex; align-items: center; justify-content: center; }}
  ...
  .donation-stats div {{ display: flex; justify-content: flex-end; align-items: center; gap: 6px; }}
  ...
  .render-icon {{ width: 1em; height: 1em; object-fit: contain; vertical-align: -0.12em; display: inline-block; }}
...
<body>
  <div class="container">
    <div class="header">
      <div class="title">{emoji_icon("📦", "donation", class_name="render-icon")} Donation Leaderboard</div>
    ...
    <div class="leaderboard">
      <h2>{emoji_icon("🏆", "trophy", class_name="render-icon")} Top Donators</h2>
      {rows_html}
    </div>
