...
body {
  ...
  background:
    ...
    radial-gradient(circle at 92% 10%, rgba(168, 86, 255, .30) 0%, rgba(168, 86, 255, 0) 26%),
...
.card::before {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(90deg, rgba(255,198,48,.95), rgba(151,70,255,.9), rgba(55,178,255,.85));
  height: 9px;
}
.header {
  position: relative;
...
.badge-wrap {
  ...
  background: radial-gradient(circle, rgba(255,215,76,.28), rgba(151,70,255,.12) 55%, rgba(0,0,0,.28));
...
.grid { position: relative; z-index: 1; display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 24px; }
.stat {
  min-height: 104px;
  border-radius: 22px;
  padding: 17px 18px;
...
.label { font-size: 15px; font-weight: 1000; opacity: .72; margin-bottom: 8px; text-transform: uppercase; letter-spacing: .8px; }
.value { font-size: 30px; line-height: 1.05; font-weight: 1000; text-shadow: 0 3px 0 rgba(0,0,0,.30); }
.value.link-value { font-size: 22px; overflow-wrap: anywhere; line-height: 1.18; }
...
CLAN_LINKS = {
    "#2CYV200G": "https://link.clashofclans.com/en?action=OpenClanProfile&tag=2CYV200G",
    "#PL0QC090": "https://link.clashofclans.com/en?action=OpenClanProfile&tag=PL0QC090",
}
...
def build_snapshot_html(clan: dict, requirements: str, clan_link: str):
    ...
    return f"""
    <html><head><style>{SNAPSHOT_CSS}</style></head>
    <body>
      <div class='card'>
        <div class='header'>
          ...
          <div class='hero-stat'><div class='hero-label'>Win Rate</div><div class='hero-value'>{win_rate}%</div></div>
        </div>
...
def register_clan_snapshot_command(tree: app_commands.CommandTree, *, get_cached_or_fetch, normalize_tag, clan_tags, clash_api_base: str = "https://api.clashofclans.com/v1"):
    ...
    async def clansnapshot(interaction: discord.Interaction, clan: app_commands.Choice[str]):
        ...
        try:
            ...
            clan_data = await get_cached_or_fetch(f"clan_snapshot_{normalized_tag}", url, ttl=120)
            if not clan_data:
            ...
            html = build_snapshot_html(clan_data, requirements, clan_link)
            buffer = await render_html_to_png_buffer(html, width=1400, height=1250, selector="body", wait_ms=700, timeout_ms=15000)
