"""Reward service helpers.

Patch 62 audit: reward logic reviewed during the low-touch service-module pass.
Reward output should stay explicit and easy to audit because this module can affect economy balance.
"""
...
def normalize_reward_bundle(reward):
    """Return a normalized reward bundle without mutating the original payload.

    Supports dict/int/string payloads so reward display, logs, and audits can
    safely describe rewards before they are applied to player state.
    """
    if isinstance(reward, dict):
        normalized = {}
        for key, value in reward.items():
            if key in {"gold", "elixir", "dark_elixir", "gems", "raid_medals", "medals", "shiny_ore", "glowy_ore", "starry_ore"}:
                try:
...
def reward_bundle_has_value(reward):
    """Return True when a normalized reward bundle contains something meaningful."""
    bundle = normalize_reward_bundle(reward)
