...
from features.phase5.equipment import (
    GEAR_CATALOG,
    HERO_ABILITIES,
    HERO_CATALOG,
    equip_hero_ability,
...
def register_economy_phase5_3_commands(bot, ctx):
    ...
    async def gear(interaction: discord.Interaction):
        ...
        for slot, item in equipment.items():
            ...
            lines.append(f"**{slot.title()}** — {format_gear_line(item)}")

        stats = get_effective_profile_stats(profile)

        embed = discord.Embed(
            title="🛡️ Equipped Gear",
            description="\n".join(lines),
            color=0xE67E22,
        )
    ...
    async def equipgear(interaction: discord.Interaction, item_id: str):
        ...
        result = equip_item(profile, item_id)
        ...
        await interaction.response.send_message(
            f"⚔️ Equipped {item_id}"
        )

    @bot.tree.command(name="heroes2", description="View unlocked heroes")
    async def heroes2(interaction: discord.Interaction):
        profile = await _profile(interaction.user)
        ...
        lines = [
            format_hero_line(hero_id, hero_data)
    ...
    async def unlockhero(interaction: discord.Interaction, hero_id: str):
        ...
        await interaction.response.send_message(
            f"🦸 Unlocked {HERO_CATALOG[hero_id]['name']}"
        )
    ...
    async def equipability(
        ...
        await interaction.response.send_message(
            f"✨ Equipped ability: {result['ability']['name']}"
        )
    ...
    async def equipgear_autocomplete(interaction: discord.Interaction, current: str):
        ...
        return [
            app_commands.Choice(
                name=f"{gear['name']} ({item_id})",
                value=item_id,
            )
            for item_id, gear in GEAR_CATALOG.items()
            if current in item_id or current in gear["name"].lower()
        ][:25]
    ...
    async def hero_autocomplete(interaction: discord.Interaction, current: str):
        ...
        return [
            ...
            for hero_id, data in HERO_CATALOG.items()
            if current in hero_id or current in data["name"].lower()
        ][:25]
    ...
    async def ability_autocomplete(interaction: discord.Interaction, current: str):
        ...
        return [
            ...
            for ability_id, data in HERO_ABILITIES.items()
            if current in ability_id or current in data["name"].lower()
        ][:25]
