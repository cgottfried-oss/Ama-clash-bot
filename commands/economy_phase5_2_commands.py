...
def register_economy_phase5_2_commands(bot, ctx):
    ...
    async def cosmetics(interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Equipped",
            value=format_equipped_cosmetics(profile),
            inline=False,
        )
    ...
    async def equipcosmetic(interaction: discord.Interaction, cosmetic_id: str):
        ...
        await interaction.response.send_message(
            f"✨ Equipped **{result['cosmetic']['name']}**"
        )
    ...
    async def grantcosmetic(
        ...
        await interaction.response.send_message(
            f"🎁 Granted **{COSMETIC_CATALOG[cosmetic_id]['name']}** to {member.mention}"
        )
