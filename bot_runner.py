...
from clash_mmo.game.state import update_mmo_state, load_mmo_state
...
# ---------------- WORLD EVENT AUTO-ROTATION ----------------

@tasks.loop(hours=2)
async def world_event_loop():
    """Rare/special auto-rotation: every 2 hours, if no world event is
    active, roll a 25% chance to start a random one. Announces in the
    Clash MMO channel when one begins."""
    try:
        from clash_mmo.game.pve.world_events import maybe_start_event, get_event_config
        from clash_mmo.game.state import update_mmo_state

        started_key = {"value": None}

        def _update(state):
            key = maybe_start_event(state)
            started_key["value"] = key
            return state

        await update_mmo_state(runtime_context, _update)

        key = started_key["value"]
        if not key:
            return
        ...
        if channel:
            embed = discord.Embed(
                ...
                description=f"{cfg.get('description', '')}\nActive for the next **24 hours**. Use `/eventstatus` to check it anytime.",
                color=0xF1C40F,
            )
            await channel.send(embed=embed)
        print(f"[WORLD EVENT] auto-started: {key}", flush=True)
...
runtime_context = {
    ...
    "SHOP_FILE": SHOP_FILE,
    "GOLD_LEADERBOARD_IMAGE_PATH": GOLD_LEADERBOARD_IMAGE_PATH,
    "SHOP_ITEMS": SHOP_ITEMS,
    "LOOT_DROP_STYLES": LOOT_DROP_STYLES,
    "CLAN_TAGS": CLAN_TAGS,
    "MAIN_CLAN_TAG": MAIN_CLAN_TAG,
